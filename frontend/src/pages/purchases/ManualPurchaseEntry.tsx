  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const purchaseData = {
        ...
        ref_date: data.metadata.invoiceDate,
        items: lineItems,
        subtotal: subtotal,
        discount_amount: discountAmount,
      ...
      // Create purchase invoice via API
      const createdInvoice = await purchaseService.createPurchaseInvoice(purchaseData);
      ...
      // Navigate to purchase register after short delay
      setTimeout(() => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader 
        ...
        description="Create a GST-compliant purchase invoice with detailed line items"
      />
              ...
              <Card className="p-6">
                <div className="space-y-3">
                  <Button 
                    type="submit" 
                    className="w-full" 
                    disabled={!hasValidLineItems || isSubmitting || vendors.length === 0}
                  >
                    {isSubmitting ? (
                      <>
